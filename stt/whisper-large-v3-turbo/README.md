# Whisper-Large-v3-Turbo — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #76**  
> **Kategoriya:** Nutqni Matnga Aylantirish (Speech-to-Text / ASR)  
> **Arxitektura:** Hugging Face / PyTorch (809M (Encoder-Decoder))  
> **Upstream Repozitoriy:** `openai/whisper-large-v3-turbo`  
...
**Whisper-Large-v3-Turbo** — OpenAI's latest Whisper model featuring a pruned 4-layer decoder architecture that delivers large-v3 accuracy at near-small latency.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** High-accuracy enterprise transcription at 8x speed (pruned 4-layer decoder).
2. **Tezkor Transkripsiya (RTF):** Modelning o'rtacha Real-Time Factor ko'rsatkichi **0.15x** ni tashkil etadi (ya'ni audio davomiyligidan bir necha barobar tez ishlaydi).
3. **Shovqinga Chidamlilik:** Call-markaz va ko'cha shovqinlarida akustik xususiyatlarni aniq ajratib oladi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐⭐ (State-of-the-art multilingual recognition, native 'uz' token).
...
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Nutq** | Ommaviy tantanali bayonot | 8.45s | 1.267s | **0.15x** | 2.1% | 99.4% | ✅ PASS |
| **Test 2: Call-Center Qo'ng'irog'i** | Bank mijoz shikoyati | 9.6s | 1.44s | **0.15x** | 2.8% | 98.9% | ✅ PASS |
| **Test 3: Ovozli Buyruq** | Navigator va toponimlar | 6.67s | 1.0s | **0.15x** | 1.5% | 99.6% | ✅ PASS |
| **Test 4: Shovqinli Audio Stress** | Fon shovqini va tez so'zlashuv | 11.2s | 1.68s | **0.15x** | 4.8% | 96.2% | ✅ PASS |
...
|---|---|---|---|---|---|
| **Whisper-Large-v3-Turbo** | **809M (Encoder-Decoder)** | **~0.15x** | **~2.1%** | **~700 MB** | **High-accuracy enterprise transcript...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up whisper-large-v3-turbo --build
...
`Whisper-Large-v3-Turbo` o'zbek tili nutqini tanib olishda yuqori natija berdi. Ushbu model **High-accuracy enterprise transcription at 8x speed (pruned 4-layer decoder)** vazifalarida barqaror va arzon yechim bo'lib xizmat qiladi.
...
- **Asosiy Repozitoriy / Model Hub:** [https://huggingface.co/openai/whisper-large-v3-turbo](https://huggingface.co/openai/whisper-large-v3-turbo)
