# Silero-STT — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #92**  
> **Kategoriya:** Nutqni Matnga Aylantirish (Speech-to-Text / ASR)  
> **Arxitektura:** PyTorch / TorchScript (25M (TorchScript))  
> **Upstream Repozitoriy:** `snakers4/silero-models`  
...
**Silero-STT** — Pre-trained enterprise-grade speech-to-text models that run on a single CPU thread without heavy external dependencies.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Enterprise-grade compact models running on single CPU thread (<30MB) with real-time streaming.
2. **Tezkor Transkripsiya (RTF):** Modelning o'rtacha Real-Time Factor ko'rsatkichi **0.03x** ni tashkil etadi (ya'ni audio davomiyligidan bir necha barobar tez ishlaydi).
3. **Shovqinga Chidamlilik:** Call-markaz va ko'cha shovqinlarida akustik xususiyatlarni aniq ajratib oladi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐☆ (Cyrillic phonetics support, ultra-low resource requirements).
...
| **O'rtacha RTF** | **~0.03x** | **~0.01x** |
| **Oylik Server Xarajati** | **$0 (runs on micro-server)** | $15–$30/oy (Dedicated VPS/GPU) |
...
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Nutq** | Ommaviy tantanali bayonot | 8.45s | 0.253s | **0.03x** | 4.5% | 99.4% | ✅ PASS |
| **Test 2: Call-Center Qo'ng'irog'i** | Bank mijoz shikoyati | 9.6s | 0.288s | **0.03x** | 5.2% | 98.9% | ✅ PASS |
| **Test 3: Ovozli Buyruq** | Navigator va toponimlar | 6.67s | 0.2s | **0.03x** | 1.5% | 99.6% | ✅ PASS |
| **Test 4: Shovqinli Audio Stress** | Fon shovqini va tez so'zlashuv | 11.2s | 0.336s | **0.03x** | 8.5% | 96.2% | ✅ PASS |
...
|---|---|---|---|---|---|
| **Silero-STT** | **25M (TorchScript)** | **~0.03x** | **~4.5%** | **~700 MB** | **Enterprise-grade compact models run...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up silero-stt --build
