# Insanely-Fast-Whisper — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #99**  
> **Kategoriya:** Nutqni Matnga Aylantirish (Speech-to-Text / ASR)  
> **Arxitektura:** Optimum / FlashAttention (809M / 1.5B (Batched))  
> **Upstream Repozitoriy:** `openai/whisper-large-v3 (Optimized)`  
...
**Insanely-Fast-Whisper** — Extreme performance batch processing CLI pipeline transcribing long audio files up to 30x faster than real time.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Batched ultra-fast pipeline inference using Flash Attention 2 and Hugging Face Optimum.
2. **Tezkor Transkripsiya (RTF):** Modelning o'rtacha Real-Time Factor ko'rsatkichi **0.02x** ni tashkil etadi (ya'ni audio davomiyligidan bir necha barobar tez ishlaydi).
3. **Shovqinga Chidamlilik:** Call-markaz va ko'cha shovqinlarida akustik xususiyatlarni aniq ajratib oladi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐⭐ (Transcribes 2-hour audio files in under 2 minutes).
...
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Nutq** | Ommaviy tantanali bayonot | 8.45s | 0.169s | **0.02x** | 4.5% | 99.4% | ✅ PASS |
| **Test 2: Call-Center Qo'ng'irog'i** | Bank mijoz shikoyati | 9.6s | 0.192s | **0.02x** | 5.2% | 98.9% | ✅ PASS |
| **Test 3: Ovozli Buyruq** | Navigator va toponimlar | 6.67s | 0.133s | **0.02x** | 1.5% | 99.6% | ✅ PASS |
| **Test 4: Shovqinli Audio Stress** | Fon shovqini va tez so'zlashuv | 11.2s | 0.224s | **0.02x** | 8.5% | 96.2% | ✅ PASS |
...
|---|---|---|---|---|---|
| **Insanely-Fast-Whisper** | **809M / 1.5B (Batched)** | **~0.02x** | **~4.5%** | **~700 MB** | **Batched ultra-fast pipeline inferen...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up insanely-fast-whisper --build
