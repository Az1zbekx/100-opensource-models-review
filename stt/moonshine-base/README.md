# Moonshine-Base — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #83**  
> **Kategoriya:** Nutqni Matnga Aylantirish (Speech-to-Text / ASR)  
> **Arxitektura:** ONNX Runtime / PyTorch (61M (Rotary Transformer))  
> **Upstream Repozitoriy:** `UsefulSensors/moonshine-base`  
...
**Moonshine-Base** — Enhanced Moonshine model providing higher vocabulary richness while preserving non-chunked variable-length streaming capability.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Low-latency live microphone speech transcription for real-time captions.
2. **Tezkor Transkripsiya (RTF):** Modelning o'rtacha Real-Time Factor ko'rsatkichi **0.09x** ni tashkil etadi (ya'ni audio davomiyligidan bir necha barobar tez ishlaydi).
3. **Shovqinga Chidamlilik:** Call-markaz va ko'cha shovqinlarida akustik xususiyatlarni aniq ajratib oladi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐☆ (Fast real-time factor and high command fidelity).
...
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Nutq** | Ommaviy tantanali bayonot | 8.45s | 0.76s | **0.09x** | 4.5% | 99.4% | ✅ PASS |
| **Test 2: Call-Center Qo'ng'irog'i** | Bank mijoz shikoyati | 9.6s | 0.864s | **0.09x** | 5.2% | 98.9% | ✅ PASS |
| **Test 3: Ovozli Buyruq** | Navigator va toponimlar | 6.67s | 0.6s | **0.09x** | 1.5% | 99.6% | ✅ PASS |
| **Test 4: Shovqinli Audio Stress** | Fon shovqini va tez so'zlashuv | 11.2s | 1.008s | **0.09x** | 8.5% | 96.2% | ✅ PASS |
...
|---|---|---|---|---|---|
| **Moonshine-Base** | **61M (Rotary Transformer)** | **~0.09x** | **~4.5%** | **~700 MB** | **Low-latency live microphone speech ...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up moonshine-base --build
...
`Moonshine-Base` o'zbek tili nutqini tanib olishda yuqori natija berdi. Ushbu model **Low-latency live microphone speech transcription for real-time captions** vazifalarida barqaror va arzon yechim bo'lib xizmat qiladi.
...
- **Asosiy Repozitoriy / Model Hub:** [https://huggingface.co/UsefulSensors/moonshine-base](https://huggingface.co/UsefulSensors/moonshine-base)
