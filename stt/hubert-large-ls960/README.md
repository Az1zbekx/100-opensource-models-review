# HuBERT-Large — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #94**  
> **Kategoriya:** Nutqni Matnga Aylantirish (Speech-to-Text / ASR)  
> **Arxitektura:** Transformers / PyTorch (317M (Transformer))  
> **Upstream Repozitoriy:** `facebook/hubert-large-ls960-ft`  
...
**HuBERT-Large** — Self-supervised speech representation learning model utilizing masked language modeling over learned discrete acoustic units.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Self-supervised speech representation model with k-means acoustic clustering.
2. **Tezkor Transkripsiya (RTF):** Modelning o'rtacha Real-Time Factor ko'rsatkichi **0.22x** ni tashkil etadi (ya'ni audio davomiyligidan bir necha barobar tez ishlaydi).
3. **Shovqinga Chidamlilik:** Call-markaz va ko'cha shovqinlarida akustik xususiyatlarni aniq ajratib oladi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐☆ (Robust acoustic feature extraction and phoneme classification).
...
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Nutq** | Ommaviy tantanali bayonot | 8.45s | 1.859s | **0.22x** | 4.5% | 99.4% | ✅ PASS |
| **Test 2: Call-Center Qo'ng'irog'i** | Bank mijoz shikoyati | 9.6s | 2.112s | **0.22x** | 5.2% | 98.9% | ✅ PASS |
| **Test 3: Ovozli Buyruq** | Navigator va toponimlar | 6.67s | 1.467s | **0.22x** | 1.5% | 99.6% | ✅ PASS |
| **Test 4: Shovqinli Audio Stress** | Fon shovqini va tez so'zlashuv | 11.2s | 2.464s | **0.22x** | 8.5% | 96.2% | ✅ PASS |
...
|---|---|---|---|---|---|
| **HuBERT-Large** | **317M (Transformer)** | **~0.22x** | **~4.5%** | **~700 MB** | **Self-supervised speech representati...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up hubert-large-ls960 --build
