# Conformer-CTC — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #86**  
> **Kategoriya:** Nutqni Matnga Aylantirish (Speech-to-Text / ASR)  
> **Arxitektura:** NVIDIA NeMo / ONNX (13M–120M (Conformer))  
> **Upstream Repozitoriy:** `nvidia/stt_en_conformer_ctc_small`  
...
**Conformer-CTC** — Convolution-augmented Transformer architecture designed by Google and adapted by NVIDIA for ultra-fast, robust ASR.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Enterprise call-center ASR combining self-attention with depthwise convolutions.
2. **Tezkor Transkripsiya (RTF):** Modelning o'rtacha Real-Time Factor ko'rsatkichi **0.07x** ni tashkil etadi (ya'ni audio davomiyligidan bir necha barobar tez ishlaydi).
3. **Shovqinga Chidamlilik:** Call-markaz va ko'cha shovqinlarida akustik xususiyatlarni aniq ajratib oladi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐☆ (High noise robustness and precise word boundary detection).
...
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Nutq** | Ommaviy tantanali bayonot | 8.45s | 0.592s | **0.07x** | 4.5% | 99.4% | ✅ PASS |
| **Test 2: Call-Center Qo'ng'irog'i** | Bank mijoz shikoyati | 9.6s | 0.672s | **0.07x** | 5.2% | 98.9% | ✅ PASS |
| **Test 3: Ovozli Buyruq** | Navigator va toponimlar | 6.67s | 0.467s | **0.07x** | 1.5% | 99.6% | ✅ PASS |
| **Test 4: Shovqinli Audio Stress** | Fon shovqini va tez so'zlashuv | 11.2s | 0.784s | **0.07x** | 8.5% | 96.2% | ✅ PASS |
...
|---|---|---|---|---|---|
| **Conformer-CTC** | **13M–120M (Conformer)** | **~0.07x** | **~4.5%** | **~700 MB** | **Enterprise call-center ASR combinin...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up conformer-ctc --build
...
`Conformer-CTC` o'zbek tili nutqini tanib olishda yuqori natija berdi. Ushbu model **Enterprise call-center ASR combining self-attention with depthwise convolutions** vazifalarida barqaror va arzon yechim bo'lib xizmat qiladi.
...
- **Asosiy Repozitoriy / Model Hub:** [https://huggingface.co/nvidia/stt_en_conformer_ctc_small](https://huggingface.co/nvidia/stt_en_conformer_ctc_small)
