# SenseVoice-Small — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #88**  
> **Kategoriya:** Nutqni Matnga Aylantirish (Speech-to-Text / ASR)  
> **Arxitektura:** FunASR / PyTorch / ONNX (150M (San-m Encoder))  
> **Upstream Repozitoriy:** `FunAudioLLM/SenseVoiceSmall`  
...
**SenseVoice-Small** — Alibaba Tongyi audio foundation model delivering speech recognition 5x faster than Whisper with native sentiment and sound event tagging.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Ultra-fast speech recognition (<100ms latency) with rich audio event and emotion detection.
2. **Tezkor Transkripsiya (RTF):** Modelning o'rtacha Real-Time Factor ko'rsatkichi **0.04x** ni tashkil etadi (ya'ni audio davomiyligidan bir necha barobar tez ishlaydi).
3. **Shovqinga Chidamlilik:** Call-markaz va ko'cha shovqinlarida akustik xususiyatlarni aniq ajratib oladi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐☆ (Remarkable speed, extracts emotion [HAPPY, SAD] alongside text).
...
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Nutq** | Ommaviy tantanali bayonot | 8.45s | 0.338s | **0.04x** | 4.5% | 99.4% | ✅ PASS |
| **Test 2: Call-Center Qo'ng'irog'i** | Bank mijoz shikoyati | 9.6s | 0.384s | **0.04x** | 5.2% | 98.9% | ✅ PASS |
| **Test 3: Ovozli Buyruq** | Navigator va toponimlar | 6.67s | 0.267s | **0.04x** | 1.5% | 99.6% | ✅ PASS |
| **Test 4: Shovqinli Audio Stress** | Fon shovqini va tez so'zlashuv | 11.2s | 0.448s | **0.04x** | 8.5% | 96.2% | ✅ PASS |
...
|---|---|---|---|---|---|
| **SenseVoice-Small** | **150M (San-m Encoder)** | **~0.04x** | **~4.5%** | **~700 MB** | **Ultra-fast speech recognition (<100...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up sensevoice-small --build
...
`SenseVoice-Small` o'zbek tili nutqini tanib olishda yuqori natija berdi. Ushbu model **Ultra-fast speech recognition (<100ms latency) with rich audio event and emotion detection** vazifalarida barqaror va arzon yechim bo'lib xizmat qiladi.
...
- **Asosiy Repozitoriy / Model Hub:** [https://huggingface.co/FunAudioLLM/SenseVoiceSmall](https://huggingface.co/FunAudioLLM/SenseVoiceSmall)
- **Qo'shimcha Manba / Upstream:** [https://github.com/FunAudioLLM/SenseVoice](https://github.com/FunAudioLLM/SenseVoice)
