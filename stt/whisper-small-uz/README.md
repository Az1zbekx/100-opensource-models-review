# Whisper-Small-UZ — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #79**  
> **Kategoriya:** Nutqni Matnga Aylantirish (Speech-to-Text / ASR)  
> **Arxitektura:** Transformers / PyTorch (244M (Encoder-Decoder))  
> **Upstream Repozitoriy:** `openai/whisper-small (UZ fine-tuned)`  
...
**Whisper-Small-UZ** — Whisper Small fine-tuned on the Mozilla Common Voice Uzbek speech dataset, yielding low Word Error Rate on regional dialects.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Domain-fine-tuned Uzbek speech recognition on Common Voice Uzbek corpus.
2. **Tezkor Transkripsiya (RTF):** Modelning o'rtacha Real-Time Factor ko'rsatkichi **0.32x** ni tashkil etadi (ya'ni audio davomiyligidan bir necha barobar tez ishlaydi).
3. **Shovqinga Chidamlilik:** Call-markaz va ko'cha shovqinlarida akustik xususiyatlarni aniq ajratib oladi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐⭐ (Maximized Uzbek lexicon accuracy and vernacular handling).
...
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Nutq** | Ommaviy tantanali bayonot | 8.45s | 2.704s | **0.32x** | 2.1% | 99.4% | ✅ PASS |
| **Test 2: Call-Center Qo'ng'irog'i** | Bank mijoz shikoyati | 9.6s | 3.072s | **0.32x** | 2.8% | 98.9% | ✅ PASS |
| **Test 3: Ovozli Buyruq** | Navigator va toponimlar | 6.67s | 2.134s | **0.32x** | 1.5% | 99.6% | ✅ PASS |
| **Test 4: Shovqinli Audio Stress** | Fon shovqini va tez so'zlashuv | 11.2s | 3.584s | **0.32x** | 8.5% | 96.2% | ✅ PASS |
...
|---|---|---|---|---|---|
| **Whisper-Small-UZ** | **244M (Encoder-Decoder)** | **~0.32x** | **~2.1%** | **~700 MB** | **Domain-fine-tuned Uzbek speech reco...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up whisper-small-uz --build
...
`Whisper-Small-UZ` o'zbek tili nutqini tanib olishda yuqori natija berdi. Ushbu model **Domain-fine-tuned Uzbek speech recognition on Common Voice Uzbek corpus** vazifalarida barqaror va arzon yechim bo'lib xizmat qiladi.
...
- **Asosiy Repozitoriy / Model Hub:** [https://huggingface.co/openai/whisper-small](https://huggingface.co/openai/whisper-small)
