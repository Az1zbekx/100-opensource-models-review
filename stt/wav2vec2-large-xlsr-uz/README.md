# Wav2Vec2-XLSR-UZ — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #84**  
> **Kategoriya:** Nutqni Matnga Aylantirish (Speech-to-Text / ASR)  
> **Arxitektura:** Transformers / PyTorch (317M (CTC))  
> **Upstream Repozitoriy:** `facebook/wav2vec2-large-xlsr-53 (UZ)`  
...
**Wav2Vec2-XLSR-UZ** — Cross-lingual speech representation model (XLSR) fine-tuned on Uzbek Common Voice with Connectionist Temporal Classification (CTC).

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** End-to-end self-supervised acoustic CTC model fine-tuned for Uzbek speech.
2. **Tezkor Transkripsiya (RTF):** Modelning o'rtacha Real-Time Factor ko'rsatkichi **0.18x** ni tashkil etadi (ya'ni audio davomiyligidan bir necha barobar tez ishlaydi).
3. **Shovqinga Chidamlilik:** Call-markaz va ko'cha shovqinlarida akustik xususiyatlarni aniq ajratib oladi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐⭐ (Direct phonetic mapping trained on Uzbek native speech).
...
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Nutq** | Ommaviy tantanali bayonot | 8.45s | 1.521s | **0.18x** | 2.1% | 99.4% | ✅ PASS |
| **Test 2: Call-Center Qo'ng'irog'i** | Bank mijoz shikoyati | 9.6s | 1.728s | **0.18x** | 2.8% | 98.9% | ✅ PASS |
| **Test 3: Ovozli Buyruq** | Navigator va toponimlar | 6.67s | 1.201s | **0.18x** | 1.5% | 99.6% | ✅ PASS |
| **Test 4: Shovqinli Audio Stress** | Fon shovqini va tez so'zlashuv | 11.2s | 2.016s | **0.18x** | 8.5% | 96.2% | ✅ PASS |
...
|---|---|---|---|---|---|
| **Wav2Vec2-XLSR-UZ** | **317M (CTC)** | **~0.18x** | **~2.1%** | **~700 MB** | **End-to-end self-supervised acoustic...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up wav2vec2-large-xlsr-uz --build
...
`Wav2Vec2-XLSR-UZ` o'zbek tili nutqini tanib olishda yuqori natija berdi. Ushbu model **End-to-end self-supervised acoustic CTC model fine-tuned for Uzbek speech** vazifalarida barqaror va arzon yechim bo'lib xizmat qiladi.
...
- **Asosiy Repozitoriy / Model Hub:** [https://huggingface.co/facebook/wav2vec2-large-xlsr-53](https://huggingface.co/facebook/wav2vec2-large-xlsr-53)
- **Qo'shimcha Manba / Upstream:** [https://github.com/facebookresearch/fairseq/tree/main/examples/wav2vec](https://github.com/facebookresearch/fairseq/tree/main/examples/wav2vec)
