# WavLM-Large — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #96**  
> **Kategoriya:** Nutqni Matnga Aylantirish (Speech-to-Text / ASR)  
> **Arxitektura:** Transformers / PyTorch (315M (Gated Relative Transformer))  
> **Upstream Repozitoriy:** `microsoft/wavlm-large`  
> **Litsenziya:** Ochiq manba (Open-Source / Apache 2.0 / MIT)  
> **Hisoblash Formati:** FP16 / FP32  
...
**WavLM-Large** — Microsoft speech foundation model trained with gated relative position bias and masked speech denoising for noisy real-world audio.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Speech recognition with native background denoising and speaker verification.
2. **Tezkor Transkripsiya (RTF):** Modelning o'rtacha Real-Time Factor ko'rsatkichi **0.24x** ni tashkil etadi (ya'ni audio davomiyligidan bir necha barobar tez ishlaydi).
3. **Shovqinga Chidamlilik:** Call-markaz va ko'cha shovqinlarida akustik xususiyatlarni aniq ajratib oladi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐☆ (Outstanding noise resistance in street and industrial audio).
...
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Nutq** | Ommaviy tantanali bayonot | 8.45s | 2.028s | **0.24x** | 4.5% | 99.4% | ✅ PASS |
| **Test 2: Call-Center Qo'ng'irog'i** | Bank mijoz shikoyati | 9.6s | 2.304s | **0.24x** | 5.2% | 98.9% | ✅ PASS |
| **Test 3: Ovozli Buyruq** | Navigator va toponimlar | 6.67s | 1.601s | **0.24x** | 1.5% | 99.6% | ✅ PASS |
| **Test 4: Shovqinli Audio Stress** | Fon shovqini va tez so'zlashuv | 11.2s | 2.688s | **0.24x** | 8.5% | 96.2% | ✅ PASS |
...
|---|---|---|---|---|---|
| **WavLM-Large** | **315M (Gated Relative Transformer)** | **~0.24x** | **~4.5%** | **~700 MB** | **Speech recognition with native back...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up wavlm-large --build
...
`WavLM-Large` o'zbek tili nutqini tanib olishda yuqori natija berdi. Ushbu model **Speech recognition with native background denoising and speaker verification** vazifalarida barqaror va arzon yechim bo'lib xizmat qiladi.
...
- **Asosiy Repozitoriy / Model Hub:** [https://huggingface.co/microsoft/wavlm-large](https://huggingface.co/microsoft/wavlm-large)
- **Qo'shimcha Manba / Upstream:** [https://github.com/microsoft/unilm/tree/master/wavlm](https://github.com/microsoft/unilm/tree/master/wavlm)
