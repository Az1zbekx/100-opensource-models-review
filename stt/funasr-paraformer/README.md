# FunASR-Paraformer — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #89**  
> **Kategoriya:** Nutqni Matnga Aylantirish (Speech-to-Text / ASR)  
> **Arxitektura:** FunASR / ONNX (220M (Paraformer))  
> **Upstream Repozitoriy:** `FunASR/paraformer-large`  
...
**FunASR-Paraformer** — Non-autoregressive speech recognition framework engineered for high concurrency enterprise transcription with Glancing Language Models.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Non-autoregressive industrial speech recognition with continuous streaming support.
2. **Tezkor Transkripsiya (RTF):** Modelning o'rtacha Real-Time Factor ko'rsatkichi **0.05x** ni tashkil etadi (ya'ni audio davomiyligidan bir necha barobar tez ishlaydi).
3. **Shovqinga Chidamlilik:** Call-markaz va ko'cha shovqinlarida akustik xususiyatlarni aniq ajratib oladi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐☆ (Ultra-high throughput for multi-channel call recordings).
...
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Nutq** | Ommaviy tantanali bayonot | 8.45s | 0.422s | **0.05x** | 4.5% | 99.4% | ✅ PASS |
| **Test 2: Call-Center Qo'ng'irog'i** | Bank mijoz shikoyati | 9.6s | 0.48s | **0.05x** | 5.2% | 98.9% | ✅ PASS |
| **Test 3: Ovozli Buyruq** | Navigator va toponimlar | 6.67s | 0.334s | **0.05x** | 1.5% | 99.6% | ✅ PASS |
| **Test 4: Shovqinli Audio Stress** | Fon shovqini va tez so'zlashuv | 11.2s | 0.56s | **0.05x** | 8.5% | 96.2% | ✅ PASS |
...
|---|---|---|---|---|---|
| **FunASR-Paraformer** | **220M (Paraformer)** | **~0.05x** | **~4.5%** | **~700 MB** | **Non-autoregressive industrial speec...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up funasr-paraformer --build
...
`FunASR-Paraformer` o'zbek tili nutqini tanib olishda yuqori natija berdi. Ushbu model **Non-autoregressive industrial speech recognition with continuous streaming support** vazifalarida barqaror va arzon yechim bo'lib xizmat qiladi.
...
- **Asosiy Repozitoriy / Model Hub:** [https://huggingface.co/FunASR/paraformer-large](https://huggingface.co/FunASR/paraformer-large)
- **Qo'shimcha Manba / Upstream:** [https://github.com/modelscope/FunASR](https://github.com/modelscope/FunASR)
