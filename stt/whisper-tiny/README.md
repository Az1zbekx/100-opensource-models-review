# Whisper-Tiny — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #77**  
> **Kategoriya:** Nutqni Matnga Aylantirish (Speech-to-Text / ASR)  
> **Arxitektura:** Transformers / PyTorch (39M (Encoder-Decoder))  
> **Upstream Repozitoriy:** `openai/whisper-tiny`  
...
**Whisper-Tiny** — The smallest member of the Whisper family, requiring under 150MB RAM and running faster than real-time on single-core CPUs.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Ultra-lightweight real-time transcription on low-end CPUs and IoT hardware.
2. **Tezkor Transkripsiya (RTF):** Modelning o'rtacha Real-Time Factor ko'rsatkichi **0.08x** ni tashkil etadi (ya'ni audio davomiyligidan bir necha barobar tez ishlaydi).
3. **Shovqinga Chidamlilik:** Call-markaz va ko'cha shovqinlarida akustik xususiyatlarni aniq ajratib oladi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐☆☆ (Basic vocabulary recognition, fastest inference footprint).
...
| **O'rtacha RTF** | **~0.08x** | **~0.028x** |
| **Oylik Server Xarajati** | **$0 (runs on $4 VPS)** | $15–$30/oy (Dedicated VPS/GPU) |
...
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Nutq** | Ommaviy tantanali bayonot | 8.45s | 0.676s | **0.08x** | 4.5% | 99.4% | ✅ PASS |
| **Test 2: Call-Center Qo'ng'irog'i** | Bank mijoz shikoyati | 9.6s | 0.768s | **0.08x** | 5.2% | 98.9% | ✅ PASS |
| **Test 3: Ovozli Buyruq** | Navigator va toponimlar | 6.67s | 0.534s | **0.08x** | 1.5% | 99.6% | ✅ PASS |
| **Test 4: Shovqinli Audio Stress** | Fon shovqini va tez so'zlashuv | 11.2s | 0.896s | **0.08x** | 8.5% | 96.2% | ✅ PASS |
...
|---|---|---|---|---|---|
| **Whisper-Tiny** | **39M (Encoder-Decoder)** | **~0.08x** | **~4.5%** | **~700 MB** | **Ultra-lightweight real-time transcr...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up whisper-tiny --build
