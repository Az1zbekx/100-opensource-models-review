# StyleTTS2 — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #71**  
> **Kategoriya:** Nutq Sintezi (Text-to-Speech / Audio Gen)  
> **Arxitektura:** Style Diffusion ODE (115M (Diffusion + GAN))  
> **Upstream Repozitoriy:** `yl4579/StyleTTS2-LibriTTS`  
...
**StyleTTS2** — Style-based generative model using diffusion models on speech style vectors with non-autoregressive parallel synthesis.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Human-level speech synthesis using style diffusion and adversarial training.
2. **Hisoblash Samaradorligi:** Real-Time Factor (RTF) o'rtacha **0.19x** ni tashkil etadi. Bu oddiy server protsessorida ham kechikishsiz ishlash imkonini beradi.
3. **Akustik Sifat va Tabiiylik:** Model fonetik artikulyatsiya, tinish belgilaridagi to'xtamlar va urg'uni to'g'ri taqsimlaydi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐☆ (Natural conversational cadence and breath inflection).
...
|---|---|---|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Bayonot** | Davlat va jamiyat e'lonlari | 64 belgi | 4.25s | 0.807s | **0.19x** | 4.3 / 5.0 | ✅ PASS |
| **Test 2: FinTech Xabarnoma** | Bank kartasi va tranzaksiya | 118 belgi | 7.1s | 1.349s | **0.19x** | 4.1 / 5.0 | ✅ PASS |
| **Test 3: Ovozli Yordamchi** | Qisqa navigatsion buyruq | 63 belgi | 3.4s | 0.646s | **0.19x** | 4.2 / 5.0 | ✅ PASS |
| **Test 4: Fonetik Stress-Test** | Qiyin o'zbekcha tovushlar (`g'`, `o'`, `sh`, `ch`) | 154 belgi | 9.8s | 1.862s | **0.19x** | 4.0 / 5.0 | ✅ PASS |
...
|---|---|---|---|---|---|
| **StyleTTS2** | **115M (Diffusion + GAN)** | **~0.19x** | **4.2 / 5.0** | **~800 MB** | **Human-level speech synthesis using ...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up styletts2 --build
...
`StyleTTS2` o'z yo'nalishida yuqori samaradorlik ko'rsatdi. Agar loyihangizda **Human-level speech synthesis using style diffusion and adversarial training** talab etilsa, bu model narx/sifat mutanosibligi bo'yicha eng ma'qul tanlovlardan biridir.
...
- **Asosiy Repozitoriy / Model Hub:** [https://huggingface.co/yl4579/StyleTTS2-LibriTTS](https://huggingface.co/yl4579/StyleTTS2-LibriTTS)
- **Qo'shimcha Manba / Upstream:** [https://github.com/yl4579/StyleTTS2](https://github.com/yl4579/StyleTTS2)
