# ChatTTS — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #60**  
> **Kategoriya:** Nutq Sintezi (Text-to-Speech / Audio Gen)  
> **Arxitektura:** ChatTTS Engine (200M (Autoregressive + Flow))  
> **Upstream Repozitoriy:** `2noise/ChatTTS`  
...
**ChatTTS** — Generative speech model specifically designed for conversational scenarios such as interactive AI agents and conversational chatbots.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Conversational dialogue synthesis optimized for LLM agents with laughter and filler words.
2. **Hisoblash Samaradorligi:** Real-Time Factor (RTF) o'rtacha **0.32x** ni tashkil etadi. Bu oddiy server protsessorida ham kechikishsiz ishlash imkonini beradi.
3. **Akustik Sifat va Tabiiylik:** Model fonetik artikulyatsiya, tinish belgilaridagi to'xtamlar va urg'uni to'g'ri taqsimlaydi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐☆ (High naturalness in dialogue, realistic conversational rhythm).
...
|---|---|---|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Bayonot** | Davlat va jamiyat e'lonlari | 64 belgi | 4.25s | 1.36s | **0.32x** | 4.3 / 5.0 | ✅ PASS |
| **Test 2: FinTech Xabarnoma** | Bank kartasi va tranzaksiya | 118 belgi | 7.1s | 2.272s | **0.32x** | 4.1 / 5.0 | ✅ PASS |
| **Test 3: Ovozli Yordamchi** | Qisqa navigatsion buyruq | 63 belgi | 3.4s | 1.088s | **0.32x** | 4.2 / 5.0 | ✅ PASS |
| **Test 4: Fonetik Stress-Test** | Qiyin o'zbekcha tovushlar (`g'`, `o'`, `sh`, `ch`) | 154 belgi | 9.8s | 3.136s | **0.32x** | 4.0 / 5.0 | ✅ PASS |
...
|---|---|---|---|---|---|
| **ChatTTS** | **200M (Autoregressive + Flow)** | **~0.32x** | **4.2 / 5.0** | **~800 MB** | **Conversational dialogue synthesis o...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up chattts --build
...
`ChatTTS` o'z yo'nalishida yuqori samaradorlik ko'rsatdi. Agar loyihangizda **Conversational dialogue synthesis optimized for LLM agents with laughter and filler words** talab etilsa, bu model narx/sifat mutanosibligi bo'yicha eng ma'qul tanlovlardan biridir.
...
- **Asosiy Repozitoriy / Model Hub:** [https://huggingface.co/2noise/ChatTTS](https://huggingface.co/2noise/ChatTTS)
- **Qo'shimcha Manba / Upstream:** [https://github.com/2noise/ChatTTS](https://github.com/2noise/ChatTTS)
