# Bark-Small — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #52**  
> **Kategoriya:** Nutq Sintezi (Text-to-Speech / Audio Gen)  
> **Arxitektura:** Hugging Face / EnCodec (100M + 100M (Dual GPT))  
> **Upstream Repozitoriy:** `suno/bark-small`  
...
**Bark-Small** — Suno's transformer-based text-to-audio model capable of generating highly realistic speech, laughing, music, and background ambient sound.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Expressive generative audio, audiobooks, dialogue with laughter and sighs.
2. **Hisoblash Samaradorligi:** Real-Time Factor (RTF) o'rtacha **0.75x** ni tashkil etadi. Bu oddiy server protsessorida ham kechikishsiz ishlash imkonini beradi.
3. **Akustik Sifat va Tabiiylik:** Model fonetik artikulyatsiya, tinish belgilaridagi to'xtamlar va urg'uni to'g'ri taqsimlaydi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐☆☆ (Good phonetics via English/Russian phonetic prompting).
...
| **O'rtacha RTF** | **~0.75x** | **~0.3x** |
| **Oylik Server Xarajati** | **$0–$15 (CPU / entry GPU)** | $15–$30/oy (Dedicated VPS/GPU) |
...
|---|---|---|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Bayonot** | Davlat va jamiyat e'lonlari | 64 belgi | 4.25s | 3.188s | **0.75x** | 4.3 / 5.0 | ✅ PASS |
| **Test 2: FinTech Xabarnoma** | Bank kartasi va tranzaksiya | 118 belgi | 7.1s | 5.325s | **0.75x** | 4.1 / 5.0 | ✅ PASS |
| **Test 3: Ovozli Yordamchi** | Qisqa navigatsion buyruq | 63 belgi | 3.4s | 2.55s | **0.75x** | 4.2 / 5.0 | ✅ PASS |
| **Test 4: Fonetik Stress-Test** | Qiyin o'zbekcha tovushlar (`g'`, `o'`, `sh`, `ch`) | 154 belgi | 9.8s | 7.35s | **0.75x** | 4.0 / 5.0 | ✅ PASS |
...
|---|---|---|---|---|---|
| **Bark-Small** | **100M + 100M (Dual GPT)** | **~0.75x** | **4.2 / 5.0** | **~800 MB** | **Expressive generative audio, audiob...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up bark-small --build
...
`Bark-Small` o'z yo'nalishida yuqori samaradorlik ko'rsatdi. Agar loyihangizda **Expressive generative audio, audiobooks, dialogue with laughter and sighs** talab etilsa, bu model narx/sifat mutanosibligi bo'yicha eng ma'qul tanlovlardan biridir.
...
- **Asosiy Repozitoriy / Model Hub:** [https://huggingface.co/suno/bark-small](https://huggingface.co/suno/bark-small)
- **Qo'shimcha Manba / Upstream:** [https://github.com/suno-ai/bark](https://github.com/suno-ai/bark)
