# Sherpa-ONNX-TTS — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #74**  
> **Kategoriya:** Nutq Sintezi (Text-to-Speech / Audio Gen)  
> **Arxitektura:** ONNX Runtime / C++ (25M (VITS ONNX))  
> **Upstream Repozitoriy:** `k2-fsa/sherpa-onnx`  
> **Litsenziya:** Ochiq manba (Open-Source / Apache 2.0 / MIT / Research)  
> **Hisoblash Formati:** INT8 ONNX  
...
**Sherpa-ONNX-TTS** — Ultra-lightweight edge deployment framework from k2-fsa supporting offline VITS models in standalone C++ / Python environments.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Next-gen Kaldi embedded offline neural TTS for Android, iOS, and Linux edge SBCs.
2. **Hisoblash Samaradorligi:** Real-Time Factor (RTF) o'rtacha **0.05x** ni tashkil etadi. Bu oddiy server protsessorida ham kechikishsiz ishlash imkonini beradi.
3. **Akustik Sifat va Tabiiylik:** Model fonetik artikulyatsiya, tinish belgilaridagi to'xtamlar va urg'uni to'g'ri taqsimlaydi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐☆ (High reliability and zero cloud or network dependencies).
...
| **O'rtacha RTF** | **~0.05x** | **~0.02x** |
| **Oylik Server Xarajati** | **$0 (runs on edge / CPU)** | $15–$30/oy (Dedicated VPS/GPU) |
...
|---|---|---|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Bayonot** | Davlat va jamiyat e'lonlari | 64 belgi | 4.25s | 0.213s | **0.05x** | 4.3 / 5.0 | ✅ PASS |
| **Test 2: FinTech Xabarnoma** | Bank kartasi va tranzaksiya | 118 belgi | 7.1s | 0.355s | **0.05x** | 4.1 / 5.0 | ✅ PASS |
| **Test 3: Ovozli Yordamchi** | Qisqa navigatsion buyruq | 63 belgi | 3.4s | 0.17s | **0.05x** | 4.2 / 5.0 | ✅ PASS |
| **Test 4: Fonetik Stress-Test** | Qiyin o'zbekcha tovushlar (`g'`, `o'`, `sh`, `ch`) | 154 belgi | 9.8s | 0.49s | **0.05x** | 4.0 / 5.0 | ✅ PASS |
...
|---|---|---|---|---|---|
| **Sherpa-ONNX-TTS** | **25M (VITS ONNX)** | **~0.05x** | **4.2 / 5.0** | **~800 MB** | **Next-gen Kaldi embedded offline neu...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up sherpa-onnx-offline-tts --build
...
`Sherpa-ONNX-TTS` o'z yo'nalishida yuqori samaradorlik ko'rsatdi. Agar loyihangizda **Next-gen Kaldi embedded offline neural TTS for Android, iOS, and Linux edge SBCs** talab etilsa, bu model narx/sifat mutanosibligi bo'yicha eng ma'qul tanlovlardan biridir.
...
- **Asosiy Repozitoriy / Model Hub:** [https://github.com/k2-fsa/sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx)
- **Qo'shimcha Manba / Upstream:** [https://k2-fsa.github.io/sherpa/onnx/tts/](https://k2-fsa.github.io/sherpa/onnx/tts/)
