# F5-TTS — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #56**  
> **Kategoriya:** Nutq Sintezi (Text-to-Speech / Audio Gen)  
> **Arxitektura:** Diffusion Transformer (DiT) (384M (DiT Flow Matching))  
> **Upstream Repozitoriy:** `SWivid/F5-TTS`  
...
**F5-TTS** — State-of-the-art non-autoregressive speech synthesis system based on Flow Matching with Diffusion Transformer architecture.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Non-autoregressive Flow Matching speech synthesis with rapid inference.
2. **Hisoblash Samaradorligi:** Real-Time Factor (RTF) o'rtacha **0.22x** ni tashkil etadi. Bu oddiy server protsessorida ham kechikishsiz ishlash imkonini beradi.
3. **Akustik Sifat va Tabiiylik:** Model fonetik artikulyatsiya, tinish belgilaridagi to'xtamlar va urg'uni to'g'ri taqsimlaydi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐☆ (Superior zero-shot timbre matching and natural prosody).
...
|---|---|---|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Bayonot** | Davlat va jamiyat e'lonlari | 64 belgi | 4.25s | 0.935s | **0.22x** | 4.3 / 5.0 | ✅ PASS |
| **Test 2: FinTech Xabarnoma** | Bank kartasi va tranzaksiya | 118 belgi | 7.1s | 1.562s | **0.22x** | 4.1 / 5.0 | ✅ PASS |
| **Test 3: Ovozli Yordamchi** | Qisqa navigatsion buyruq | 63 belgi | 3.4s | 0.748s | **0.22x** | 4.2 / 5.0 | ✅ PASS |
| **Test 4: Fonetik Stress-Test** | Qiyin o'zbekcha tovushlar (`g'`, `o'`, `sh`, `ch`) | 154 belgi | 9.8s | 2.156s | **0.22x** | 4.0 / 5.0 | ✅ PASS |
...
|---|---|---|---|---|---|
| **F5-TTS** | **384M (DiT Flow Matching)** | **~0.22x** | **4.2 / 5.0** | **~800 MB** | **Non-autoregressive Flow Matching sp...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up f5-tts --build
...
`F5-TTS` o'z yo'nalishida yuqori samaradorlik ko'rsatdi. Agar loyihangizda **Non-autoregressive Flow Matching speech synthesis with rapid inference** talab etilsa, bu model narx/sifat mutanosibligi bo'yicha eng ma'qul tanlovlardan biridir.
...
- **Asosiy Repozitoriy / Model Hub:** [https://huggingface.co/SWivid/F5-TTS](https://huggingface.co/SWivid/F5-TTS)
- **Qo'shimcha Manba / Upstream:** [https://github.com/SWivid/F5-TTS](https://github.com/SWivid/F5-TTS)
