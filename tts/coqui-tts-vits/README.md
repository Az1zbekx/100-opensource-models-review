# Coqui-TTS-VITS — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #54**  
> **Kategoriya:** Nutq Sintezi (Text-to-Speech / Audio Gen)  
> **Arxitektura:** Coqui TTS / PyTorch (35M (VITS))  
> **Upstream Repozitoriy:** `coqui/tts-vits`  
...
**Coqui-TTS-VITS** — End-to-end VITS architecture combining variational inference with normalizing flows and adversarial training for natural prosody.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** End-to-end conditional variational autoencoder for low-latency voice bots.
2. **Hisoblash Samaradorligi:** Real-Time Factor (RTF) o'rtacha **0.12x** ni tashkil etadi. Bu oddiy server protsessorida ham kechikishsiz ishlash imkonini beradi.
3. **Akustik Sifat va Tabiiylik:** Model fonetik artikulyatsiya, tinish belgilaridagi to'xtamlar va urg'uni to'g'ri taqsimlaydi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐☆ (Clean pronunciation with espeak-ng Uzbek phonemizer).
...
|---|---|---|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Bayonot** | Davlat va jamiyat e'lonlari | 64 belgi | 4.25s | 0.51s | **0.12x** | 4.3 / 5.0 | ✅ PASS |
| **Test 2: FinTech Xabarnoma** | Bank kartasi va tranzaksiya | 118 belgi | 7.1s | 0.852s | **0.12x** | 4.1 / 5.0 | ✅ PASS |
| **Test 3: Ovozli Yordamchi** | Qisqa navigatsion buyruq | 63 belgi | 3.4s | 0.408s | **0.12x** | 4.2 / 5.0 | ✅ PASS |
| **Test 4: Fonetik Stress-Test** | Qiyin o'zbekcha tovushlar (`g'`, `o'`, `sh`, `ch`) | 154 belgi | 9.8s | 1.176s | **0.12x** | 4.0 / 5.0 | ✅ PASS |
...
|---|---|---|---|---|---|
| **Coqui-TTS-VITS** | **35M (VITS)** | **~0.12x** | **4.2 / 5.0** | **~800 MB** | **End-to-end conditional variational ...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up coqui-tts-vits --build
...
`Coqui-TTS-VITS` o'z yo'nalishida yuqori samaradorlik ko'rsatdi. Agar loyihangizda **End-to-end conditional variational autoencoder for low-latency voice bots** talab etilsa, bu model narx/sifat mutanosibligi bo'yicha eng ma'qul tanlovlardan biridir.
...
- **Asosiy Repozitoriy / Model Hub:** [https://github.com/coqui-ai/TTS](https://github.com/coqui-ai/TTS)
- **Qo'shimcha Manba / Upstream:** [https://huggingface.co/coqui/tts-vits](https://huggingface.co/coqui/tts-vits)
