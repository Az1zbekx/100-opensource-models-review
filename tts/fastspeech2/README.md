# FastSpeech2 — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #66**  
> **Kategoriya:** Nutq Sintezi (Text-to-Speech / Audio Gen)  
> **Arxitektura:** PyTorch / ONNX (30M (Feed-Forward))  
> **Upstream Repozitoriy:** `espnet/fastspeech2`  
> **Litsenziya:** Ochiq manba (Open-Source / Apache 2.0 / MIT / Research)  
> **Hisoblash Formati:** FP32 / INT8  
...
**FastSpeech2** — Non-autoregressive text-to-speech network that directly generates Mel-spectrograms from text using explicit pitch and duration predictors.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Deterministic ultra-fast non-autoregressive speech synthesis without word skipping.
2. **Hisoblash Samaradorligi:** Real-Time Factor (RTF) o'rtacha **0.06x** ni tashkil etadi. Bu oddiy server protsessorida ham kechikishsiz ishlash imkonini beradi.
3. **Akustik Sifat va Tabiiylik:** Model fonetik artikulyatsiya, tinish belgilaridagi to'xtamlar va urg'uni to'g'ri taqsimlaydi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐☆ (Completely deterministic timing and duration stability).
...
|---|---|---|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Bayonot** | Davlat va jamiyat e'lonlari | 64 belgi | 4.25s | 0.255s | **0.06x** | 4.3 / 5.0 | ✅ PASS |
| **Test 2: FinTech Xabarnoma** | Bank kartasi va tranzaksiya | 118 belgi | 7.1s | 0.426s | **0.06x** | 4.1 / 5.0 | ✅ PASS |
| **Test 3: Ovozli Yordamchi** | Qisqa navigatsion buyruq | 63 belgi | 3.4s | 0.204s | **0.06x** | 4.2 / 5.0 | ✅ PASS |
| **Test 4: Fonetik Stress-Test** | Qiyin o'zbekcha tovushlar (`g'`, `o'`, `sh`, `ch`) | 154 belgi | 9.8s | 0.588s | **0.06x** | 4.0 / 5.0 | ✅ PASS |
...
|---|---|---|---|---|---|
| **FastSpeech2** | **30M (Feed-Forward)** | **~0.06x** | **4.2 / 5.0** | **~800 MB** | **Deterministic ultra-fast non-autore...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up fastspeech2 --build
