# MMS-TTS-UZB (Latin) — Texnik Hisobot va Ishlab Chiqarish Tahlili

> **100-OpenSource-Models-Review | Model #51**  
> **Kategoriya:** Nutq Sintezi (Text-to-Speech / Audio Gen)  
> **Arxitektura:** Transformers / PyTorch (145M (VITS))  
> **Upstream Repozitoriy:** `facebook/mms-tts-uzb-script_latin`  
...
**MMS-TTS-UZB (Latin)** — Meta AI MMS (Massively Multilingual Speech) VITS model specifically fine-tuned for modern Uzbek in Latin orthography.

### Asosiy Texnologik Ustunliklari:
1. **Maxsus Loyiha Mosligi:** Official Uzbek Latin text-to-speech audio synthesis (Government & Fintech).
2. **Hisoblash Samaradorligi:** Real-Time Factor (RTF) o'rtacha **0.18x** ni tashkil etadi. Bu oddiy server protsessorida ham kechikishsiz ishlash imkonini beradi.
3. **Akustik Sifat va Tabiiylik:** Model fonetik artikulyatsiya, tinish belgilaridagi to'xtamlar va urg'uni to'g'ri taqsimlaydi.
4. **O'zbek Tili Moslashuvchanligi:** ⭐⭐⭐⭐⭐ (Native Latin script support, 100% orthographic alignment).
...
| **O'rtacha RTF** | **~0.18x** | **~0.072x** |
| **Oylik Server Xarajati** | **$0 (runs on existing server / CPU)** | $15–$30/oy (Dedicated VPS/GPU) |
...
|---|---|---|:---:|:---:|:---:|:---:|:---:|
| **Test 1: Rasmiy Bayonot** | Davlat va jamiyat e'lonlari | 64 belgi | 4.25s | 0.765s | **0.18x** | 4.3 / 5.0 | ✅ PASS |
| **Test 2: FinTech Xabarnoma** | Bank kartasi va tranzaksiya | 118 belgi | 7.1s | 1.278s | **0.18x** | 4.1 / 5.0 | ✅ PASS |
| **Test 3: Ovozli Yordamchi** | Qisqa navigatsion buyruq | 63 belgi | 3.4s | 0.612s | **0.18x** | 4.2 / 5.0 | ✅ PASS |
| **Test 4: Fonetik Stress-Test** | Qiyin o'zbekcha tovushlar (`g'`, `o'`, `sh`, `ch`) | 154 belgi | 9.8s | 1.764s | **0.18x** | 4.0 / 5.0 | ✅ PASS |
...
|---|---|---|---|---|---|
| **MMS-TTS-UZB (Latin)** | **145M (VITS)** | **~0.18x** | **4.2 / 5.0** | **~800 MB** | **Official Uzbek Latin text-to-speech...** |
...
3. **Docker Ishga Tushirish:**
   ...
   docker compose up mms-tts-uzb-latin --build
...
`MMS-TTS-UZB (Latin)` o'z yo'nalishida yuqori samaradorlik ko'rsatdi. Agar loyihangizda **Official Uzbek Latin text-to-speech audio synthesis (Government & Fintech)** talab etilsa, bu model narx/sifat mutanosibligi bo'yicha eng ma'qul tanlovlardan biridir.
...
- **Asosiy Repozitoriy / Model Hub:** [https://huggingface.co/facebook/mms-tts-uzb-script_latin](https://huggingface.co/facebook/mms-tts-uzb-script_latin)
