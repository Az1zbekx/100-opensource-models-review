...
Hugging Face's compact flagship on-device language model.
Trained on 11T tokens of Cosmopedia v2, FineWeb-Edu, and curated synthetic data.
Optimized for ultra-fast local edge execution on CPU and low-power hardware.
"""
...
def format_smollm_prompt(user_text: str, system_prompt: str = "You are a helpful, concise AI assistant optimized for on-device reasoning and swift responses.") -> str:
    """Format prompt with ChatML template used by SmolLM2."""
...
def run_inference(llm: Llama, user_prompt: str, max_tokens: int = 1024, temperature: float = 0.3):
    """Run inference."""
    full_prompt = format_smollm_prompt(user_prompt)
...
def main():
    parser = argparse.ArgumentParser(description="SmolLM2-1.7B-Instruct Inference Runner")
    ...
    if args.input_file:
        ...
        prompt_text = "Explain how WebSockets achieve full-duplex communication compared to HTTP long-polling in 3 bullet points."
